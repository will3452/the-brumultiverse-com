...
<a href="{{$fixing ? '#' : $href}}" x-data="{isHover:false}" x-on:mouseover="isHover = true" x-on:mouseout="isHover = false" class="overflow-hidden relative hover:shadow-xl hover:bg-base-200 block w-32 h-32  rounded-xl border m-2 p-2 flex items-center  justify-center">
    @if($fixing)
